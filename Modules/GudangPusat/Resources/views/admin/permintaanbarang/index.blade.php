...
@section('content')
    <section class="section">
        ...
        <div class="section-body">
            <h2 class="section-title">@lang('gudangpusat::permintaanbarang.permintaanbarang_list')</h2>
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>@lang('gudangpusat::permintaanbarang.manage_permintaanbarangs')</h4>
                        </div>
                        <div class="card-body">
                            @include('gudangpusat::admin.shared.flash')
                            @include('gudangpusat::admin.permintaanbarang._filter')
                            <div class="table-responsive">
                                <table class="table table-bordered text-center">
                                    <thead>
                                        <th>@lang('gudangpusat::permintaanbarang.nama_cabang_label')</th>
                                        <th>@lang('gudangpusat::permintaanbarang.nama_barang_label')</th>
                                        <th>@lang('gudangpusat::permintaanbarang.kategori_barang_label')</th>
                                        <th>@lang('gudangpusat::permintaanbarang.harga_barang_label')</th>
                                        <th>@lang('gudangpusat::permintaanbarang.permintaan_order_label')</th>
                                        <th>@lang('gudangpusat::permintaanbarang.status_order_label')</th>
                                        <th>@lang('gudangpusat::permintaanbarang.updated_at_label')</th>
                                        @if (Gate::check('edit_gudang_pusat-permintaan_order') || Gate::check('delete_gudang_pusat-permintaan_order'))
                                        ...
                                        @forelse ($permintaanbarangs as $permintaanbarang)
                                            <tr>
                                                ...
                                                <td class="text-nowrap">
                                                    @can('edit_gudang_pusat-permintaan_order')
                                                        @if ($permintaanbarang->status == 'menunggu konfirmasi' || $permintaanbarang->status == 'order dipending')
                                                            <a href="{{ url('admin/gudang-pusat/permintaan-order/' . $permintaanbarang->id . '?status=sedang-diproses') }}"
                                                        ...
                                                        @if ($permintaanbarang->status == 'sedang diproses')
                                                            <a href="{{ url('admin/gudang-pusat/permintaan-order/' . $permintaanbarang->id . '?status=sedang-dalam-pengiriman') }}"
                                                                class="btn btn-sm btn-success"
                                                                onclick="
                                                            return (confirm('Ingin mengubah status orderan ini?'))">
                                                        ...
                                                        @if ($permintaanbarang->status == 'menunggu konfirmasi')
                                                            <a href="{{ url('admin/gudang-pusat/permintaan-order/' . $permintaanbarang->id . '?status=order-dipending') }}"
