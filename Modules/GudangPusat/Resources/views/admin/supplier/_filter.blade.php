@php
    $route = 'admin/gudang-pusat/supplier';
    if ($viewTrash) {
        $route = 'admin/gudang-pusat/supplier/trashed';
    }
@endphp
...
<div class="form-row">
    ...
    <div class="form-group col-md-2">
        @can('add_gudang_pusat-supplier')
            <a href="{{ url('admin/gudang-pusat/supplier/create') }}"
                class="btn btn-icon btn-block icon-left btn-success btn-filter"><i class="fas fa-plus-circle"></i>
                @lang('gudangpusat::supplier.btn_create_label')</a>
        @endcan
    </div>
    <div class="form-group col-md-2">
        @can('delete_gudang_pusat-supplier')
            @if (!$viewTrash)
                <a href="{{ url('admin/gudang-pusat/supplier/trashed') }}"
                    class="btn btn-icon btn-block icon-left btn-light btn-filter"><i class="fas fa-trash"></i>
                    @lang('gudangpusat::supplier.btn_show_trashed_label')</a>
            @endif
        @endcan
    </div>
</div>
